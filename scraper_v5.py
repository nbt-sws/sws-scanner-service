#!/usr/bin/env python3
"""
TCG Card Scraper - One Piece TCG (Complete v5 with Full Image Fallback)
Fetches ALL card data including detailed promo variants.
Uses allPromos flat list as fallback for images when /api/promos/card/{id}/ returns null.
"""
import os
import re
import time
...
# â”€â”€ Build comprehensive image lookup â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
def build_image_lookup(all_set_cards, all_st_cards, all_don, all_promos):
    """Build a lookup of card_image_id -> card_image from ALL endpoints."""
    lookup = {}
    for item in all_set_cards + all_st_cards + all_don + all_promos:
        image_id = item.get("card_image_id")
        image_url = item.get("card_image")
        if image_id and image_url and image_id not in lookup:
            lookup[image_id] = image_url
    return lookup

# â”€â”€ Process a single card item â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def main():
    print("TCG Card Scraper - One Piece TCG (Complete v5 with Full Image Fallback)")
    ...
    # Build image lookup from ALL endpoints
    print("\n=== Building comprehensive image lookup ===")
    image_lookup = build_image_lookup(all_set_cards, all_st_cards, all_don, all_promos)
    ...
    for idx, p_id in enumerate(p_ids, 1):
        ...
        for v in variants:
            ...
            # Fallback 1: image lookup from all endpoints
        ...
        if idx % 20 == 0 or idx == len(p_ids):
            print(f"  [{idx}/{len(p_ids)}] processed")
